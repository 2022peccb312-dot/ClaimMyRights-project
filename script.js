...
function formatGeminiText(text = "") {
    if (!text) return "";
    // Only format for display in popups (other modules), keep complaint plain text
...
function speakText(text) {
    ...
    // Slower & clearer for Indian languages
...
async function sendRequest(endpoint, payload, title, loadingText) {
    // For complaint, show in popup textarea
    if (endpoint === "complaint") {
        const aiTextarea = document.getElementById("generatedComplaint");
        aiTextarea.value = loadingText;
        document.getElementById("downloadBtn").disabled = true; // disable download until AI done
...
document.getElementById("aidBtn")?.addEventListener("click", () => {
    ...
    if (!category || !state || !issue) return showPopup("Government Aid", "Please fill Category, State, and Problem details.");
...
function downloadComplaint() {
    ...
    if (!name || !aiText) { alert("No complaint generated."); return; }
